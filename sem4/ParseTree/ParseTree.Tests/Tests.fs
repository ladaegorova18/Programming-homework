...
let ``Division test`` x y = parseTree (Divide(Int(x), Int(y))) |> should equal (x / y)

[<Test>]
let fsCheckSumTest () = Check.Quick ``Sum test``

[<Test>]
let fsCheckMultiplyTest () = Check.Quick ``Multiplication test``

[<Test>]
let fsCheckSubtractionTest () = Check.Quick ``Subtraction test``

[<Test>]
let fsCheckDivisionTest () = Check.Quick ``Division test``
